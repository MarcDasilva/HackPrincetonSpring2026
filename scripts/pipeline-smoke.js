...
async function main() {
  ...
  console.log(`[SMOKE] Python: ${pythonBin}`);
  console.log(`[SMOKE] Minecraft target: ${MC_HOST}:${MC_PORT} (reachable=${mcReachable})`);
  console.log(`[SMOKE] Bridge port: ${bridgePort}`);

  if (!fs.existsSync(path.join(REPO_ROOT, "voyager", "__init__.py"))) {
    throw new Error("Voyager package not found at repo root.");
  }
  ...
  const ckptDir = path.join(REPO_ROOT, "ckpt-pipeline-smoke");
  fs.mkdirSync(ckptDir, { recursive: true });
  const tempPy = path.join(os.tmpdir(), `voyager-smoke-${Date.now()}.py`);
  ...
  const pyScript = [
    ...
    "  resume=False,",
    ")",
    "print('[SMOKE] Voyager init OK')",
    "voyager.close()",
    "print('[SMOKE] Voyager close OK')",
  ].join("\n");

  fs.writeFileSync(tempPy, pyScript);

  await new Promise((resolve, reject) => {
    const child = spawn(pythonBin, [tempPy], { cwd: REPO_ROOT, env: { ...process.env } });
    let stderr = "";
    ...
    child.on("close", (code) => {
      ...
      if (code === 0) resolve();
      else reject(new Error(`Smoke run failed with code ${code}. ${stderr.slice(-500)}`));
    });
  });
}
