...
async function ensureAgentsDocumentLoaded(loadDocs) {
...
async function queryAgentDirectives(query) {
  const supabase = getSupabase();
...
async function seedWorld(context) {
  const supabase = getSupabase();
  const rows = [
    {
...
async function registerAgents(context, agents) {
  const supabase = getSupabase();
...
async function createUserMessage(context) {
  const supabase = getSupabase();
  const { error } = await supabase
    .from('chat_messages')
    .insert({
...
async function createPendingJob(context) {
  const supabase = getSupabase();
  const { error } = await supabase
    .from('jobs_history')
    .insert({
...
async function addAgentMemory(agentId, runId, memoryType, content) {
  const supabase = getSupabase();
  const { error } = await supabase
    .from('agent_memory')
    .insert({
...
async function getNearestWorldObject(type, origin) {
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from('world_objects')
...
async function assignJob(jobId, agentId) {
  const supabase = getSupabase();
  const { error } = await supabase
    .from('jobs_history')
    .update({
...
async function sendChatMessage(sender, content, metadata) {
  const supabase = getSupabase();
  const { error } = await supabase
    .from('chat_messages')
    .insert({
...
async function updateWorldAfterMining(runId, ore, chest, agentId) {
  const supabase = getSupabase();
...
async function getRecentAgentMemories(agentId, runId) {
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from('agent_memory')
...
async function readAgentStatuses(agentIds) {
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from('agent_status')
    .select('*')
...
async function readRunChatMessages(runId) {
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from('chat_messages')
    .select('*')
...
async function simulateAgent(agent, context) {
  const redis = getRedis();
...
async function runAssertions(context, agents) {
  const redis = getRedis();
...
async function main() {
  ...
  try {
    ...
    if (!options.keepData) {
      await cleanupRun(context).catch((error) => {
